"""Research Loop — the core brain of Sentinel.
...
from sentinel_v2.app_model import (
    ApplicationModel, BusinessObject, AttackOpportunity,
    TrustBoundary, WorkflowStep, ProductType,
)
from sentinel_v2.memory import Memory, Finding, Hypothesis, Severity
...
"""
        ...
        try:
            ...
            for role_data in data.get("user_roles", []):
                from sentinel_v2.app_model import UserRole
...
"""
    ...
    async def _execute_http(self, action: dict) -> dict[str, Any]:
        ...
        if not result["error"] and result["status_code"] > 0:
            from sentinel_v2.detectors import Detector
            detector = Detector()
    ...
    async def _execute_replay(self, action: dict) -> dict[str, Any]:
        ...
        from sentinel_v2.tools.replay import HTTPReplay
