...
class BenchmarkRunner:
    ...
    async def run_all(self, hunt_fn) -> list[BenchmarkResult]:
        for target in BENCHMARK_TARGETS:
            result = await self.run_target(target, hunt_fn)
            self.results.append(result)
        self._save_results()
        return self.results

    async def run_target(
        self, target: BenchmarkTarget, hunt_fn
    ) -> BenchmarkResult:
        start = time.time()
        actual_findings = await hunt_fn(target.url)
